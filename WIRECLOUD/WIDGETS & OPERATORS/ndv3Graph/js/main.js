...
function Controlador() {
	...
	if (this.url !== null) {
		...
		if (this.flagWorkspace === "wirecloud") {
		...
		logg("init", "Se requiere URL e Identificador para iniciar. Configure y Reinicie el Widget!!.", 82);
	}

}

Controlador.prototype.settingToLocal = function() {
...
Controlador.prototype.settingToWirecloud = function() {
...
Controlador.prototype.getIdentificadorWebSocket = function() {
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.noData = function(msg) {
	//$("#msg").empty();
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.presentarDatos = function(data) {
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.pieChart = function(data) {
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.discreteBarChart = function(data) {
	if (data === null) {
		//default data
		data = [{
	...
	chart.discretebar.dispatch.on("elementClick", function(e) {
		...
		if (boolPresentacionWirecloud) {
			//wiring the chromosome number
			MashupPlatform.wiring.pushEvent('outputVariable', JSON.stringify(obj));
		}
		if (ws) {
			ws.conn.send(JSON.stringify(obj));
		}
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.transformadorDataPieChar = function(datos) {
	var bandera = false;
	var json;
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.transformadorDataDiscreteBarChar = function(datos) {
	var bandera = false;
	var json;
	...
	try {
		...
		json = JSON.parse(datos);
		logg("transformadorData", "Numero de datos :" + json.legth, 382);
		//preparar json en formato que absorve el gráfico
		for (var i = 0; i < json.length; i++) {
			var v = new Object();
			v.label = json[i].id;
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.crearWebSocket = function() {
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.dispararCambio = function(cadena) {
 ...
 * @author Carlos iniguez
 */
Controlador.prototype.logg = function(nombreFuncion, mensaje, linea) {
	if (environment === "dev") {
...
document.addEventListener('load', new Controlador(), true);
