...
(function() {
	...
	nv.interactiveGuideline = function() {
		...
		function layer(selection) {
			selection.each(function(data) {
				...
				svgContainer.on("touchmove", mouseHandler).on("mousemove", mouseHandler, true).on("mouseout", mouseHandler, true).on("dblclick", mouseHandler).on("click", mouseHandler);
	...
	(function() {
		...
		nv.models.tooltip = function() {
			...
			var contentGenerator = function(d) {
				...
				trowEnter.append("td").classed("key", true).html(function(p, i) {
			...
			var calcTooltipPosition = function(pos) {
				...
				nv.dom.read(function() {
					...
					case 'e':
						...
						if (tLeft < scrollLeft)
							left = pos[0] + distance > scrollLeft ? pos[0] + distance : scrollLeft - tLeft + left;
						if (tTop < scrollTop)
					...
					if (hidden) {
						tooltip.transition().delay(hideDelay).duration(0).style('opacity', 0);
					} else {
						...
						.style('-webkit-transform', new_translate).style('opacity', 1);
					}
			...
			function initTooltip() {
				if (!tooltip) {
					...
					tooltip = d3.select(body).append("div").attr("class", "nvtooltip " + ( classes ? classes : "xy-tooltip")).attr("id", id);
	...
	nv.utils.renderWatch = function(dispatch, duration) {
		...
		this.transition = function(selection, args, duration) {
			...
			if (duration === 0) {
				...
				var n = 0;
				return selection.transition().duration(duration).each(function() {++n;
	...
	nv.models.boxPlot = function() {
		...
		function chart(selection) {
			...
			selection.each(function(data) {
				...
				outliers.watchTransition(renderWatch, 'nv-boxplot: nv-boxplot-outlier').attr('cx', x.rangeBand() * .45).attr('cy', function(d, i, j) {
					return y(d);
				}).attr('r', '3');
				...
				['low', 'high'].forEach(function(key) {
					...
					boxplots.select('line.nv-boxplot-whisker.nv-boxplot-' + key).watchTransition(renderWatch, 'nv-boxplot: boxplots').attr('x1', x.rangeBand() * .45).attr('y1', function(d, i) {
						return y(d.values['whisker_' + key]);
					}).attr('x2', x.rangeBand() * .45).attr('y2', function(d, i) {
	...
	nv.models.candlestickBar = function() {
		...
		function chart(selection) {
			selection.each(function(data) {
				...
				var tickGroups = ticks.enter().append('g').attr('class', function(d, i, j) {
				...
				container.selectAll('.nv-candlestick-lines').transition().attr('transform', function(d, i) {
					return 'translate(' + x(getX(d, i)) + ',0)';
				}).attr('x1', 0).attr('y1', function(d, i) {
					return y(getHigh(d, i));
				}).attr('x2', 0).attr('y2', function(d, i) {
				...
				container.selectAll('.nv-candlestick-rects').transition().attr('transform', function(d, i) {
					return 'translate(' + (x(getX(d, i)) - barWidth / 2) + ',' + (y(getY(d, i)) - (getOpen(d, i) > getClose(d, i) ? (y(getClose(d, i)) - y(getOpen(d, i))) : 0)) + ')';
				}).attr('x', 0).attr('y', 0).attr('width', barWidth).attr('height', function(d, i) {
					var open = getOpen(d, i);
					var close = getClose(d, i);
	...
	nv.models.discreteBarChart = function() {
		...
		function chart(selection) {
			...
			if (showYAxis)
				renderWatch.models(yAxis);
				//TODO: Puesto por Carlos para eliminar error de consola
		...
		function chart(selection) {
			selection.each(function(data) {
				...
				dimensionNames.forEach(function(d) {
					...
					y[d] = d3.scale.linear().domain(extent).range([(availableHeight - 12) * 0.9, 0]);
				...
				var line = d3.svg.line().interpolate('cardinal').tension(lineTension),
				    ...
				    axisDrag = d3.behavior.drag().on('dragstart', dragStart).on('drag', dragMove).on('dragend', dragEnd);

				//Add missing value line at the bottom of the chart
				var missingValuesline,
	...
	nv.models.pieChart = function() {
		...
		function chart(selection) {
			...
			//TODO: Control puesto por carlos para quitar error de consola
			if (selection === undefined) {
				return null;
			}
			//------------------------
					...
					if (useVoronoi === true) {
						var vertices = d3.merge(data.map(function(group, groupIndex) {
							return group.values.map(function(point, pointIndex) {
								...
								return [x(pX) + Math.random() * 1e-4, y(pY) + Math.random() * 1e-4, groupIndex, pointIndex, point];
						...
						if (clipVoronoi) {
							...
							wrap.select('.nv-point-clips').selectAll("clipPath").data(vertices).enter().append("svg:clipPath").attr("id", function(d, i) {
								return "nv-clip-" + i;
		...
		function chart(selection) {
			...
			selection.each(function(data) {
				...
				scatter.dispatch.on('elementMouseover.tooltip', function(evt) {
					container.select('.nv-series-' + evt.seriesIndex + ' .nv-distx-' + evt.pointIndex).attr('y1', evt.pos.top - availableHeight - margin.top);
					container.select('.nv-series-' + evt.seriesIndex + ' .nv-disty-' + evt.pointIndex).attr('x2', evt.pos.left + distX.size() - margin.left);
			...
			selection.each(function(data) {
				...
				chart.d3_stackedOffset_stackPercent = function(stackData) {
					...
					for ( j = 0; j < m; ++j) {//Looping through all points
						for ( i = 0,
						o = 0; i < dataRaw.length; i++) {//looping through all series
							o += getY(dataRaw[i].values[j]);
							//total y value of all series at a certian point in time.
						}
